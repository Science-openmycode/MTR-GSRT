"""Fixed-seed development release for a low-dimensional distance-corridor query."""
...
BLOCK_BUDGETS = {
    ...
    "distance_coarse24_flow": 350_000,
}
if sum(BLOCK_BUDGETS.values()) != ROUTE_LATTICE:
    raise RuntimeError("Distance-corridor block budgets must sum to Q")
...
def empty_blocks(context96, context384) -> dict[str, np.ndarray]:
    return {
        ...
        "distance_coarse24_flow": np.zeros(
            (4, len(context96.coarse_edge_index))
        ),
    }
...
def trajectory_blocks(trajectory, context96, context384) -> dict[str, np.ndarray]:
    ...
    return {
        ...
        "distance_coarse24_flow": query._transitions(
            labels24,
...
def main() -> None:
    ...
    if args.exact_query is not None:
        ...
        for index, trajectory in enumerate(real):
            ...
            if (index + 1) % 1000 == 0:
                print(f"[distance-corridor-query] {index + 1}/{capacity}", flush=True)
    ...
    noisy, sampler = certified.add_exact_discrete_laplace(
        ...
        rng=random.Random(int(args.seed) + 440_019),
    )
    released, offset = {}, 0
    for name, budget in BLOCK_BUDGETS.items():
    ...
    report = {
        ...
        "sampler": sampler,
        "design": (
            "The 350000-unit OD block is 4 trip-distance families by directed "
            "edges of the public 24-region quotient graph."
        ),
    }
