...
def synthesize(real, method, *, bbox=None, osm_ways=None, raw_graph=False):
    ...
    # Pass attempts explicitly: Python binds default arguments at function
    # definition time, so changing EXP_N_GEN for full-mode reruns would
    # otherwise still use the original small default.
    draws, meas = sample_joint_draws(norm_train, centers_norm, rng, attempts=max(EXP_N_GEN * 4, EXP_N_GEN + 100), pgm=(method == "dpPGM"))
    route_specs = []
    for o, d, lb in draws:
