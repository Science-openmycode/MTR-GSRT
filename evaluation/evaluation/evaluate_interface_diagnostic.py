...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        ...
        action="store_true",
        required=True,
        help="Explicitly acknowledge that this external per-record interface does not satisfy the fixed public-slot population contract.",
    )
...
def _resolve_config(args: argparse.Namespace) -> dict:
    ...
    return {
        ...
        "public_slot_count": int(config["public_slot_count"]),
    }
...
def main() -> None:
    ...
    if len(synthetic) != config["public_slot_count"] and not args.interface_diagnostic:
        raise RuntimeError(
