...
def fit_measurements(train, centers, rng, *, pgm=False, eps_total=EPS, eps_anchor=0.20):
    """Release fixed-support trip-condition measurements with explicit budget.

    Defaults reproduce the original ledger (EPS=1.0, anchor=0.20).  Public
    DP-GSRT passes the budget explicitly so the reported epsilon controls the
    actual Laplace scales.
    """
