...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--emit-matched-candidate-bank",
        action="store_true",
        help="Research-only: serialize the ordered public-only proposal bank.",
    )
    parser.add_argument(
        "--matched-candidate-bank",
        type=Path,
        help="Research-only: reuse the exact public-only ordered proposal bank.",
    )
...
MATCHED_CANDIDATE_SCHEMA = "mtr-matched-candidate-bank-v1"
MATCHED_SCORE_FORMULA = (
    "length_logit + mean_log_q5_portal_probability(recognized boundary crossings) "
    "- mean_log_public_only_portal_probability(same crossings)"
)


def candidate_reference_score(nodes: np.ndarray, labels: np.ndarray, reference: np.ndarray) -> float:
    """Length-normalized log score of one fixed road path under a quotient kernel."""
    regions = np.asarray(labels[np.asarray(nodes, dtype=int)], dtype=np.int32)
    if regions.size < 2:
        return 0.0
    keep = np.r_[True, regions[1:] != regions[:-1]]
    regions = regions[keep]
    if regions.size < 2:
        return 0.0
    probabilities = np.asarray(reference[regions[:-1], regions[1:]], dtype=float)
    probabilities = np.clip(probabilities, np.finfo(float).tiny, 1.0)
    return float(np.log(probabilities).mean())


def candidate_portal_score(
    nodes: np.ndarray,
    labels: np.ndarray,
    coords: np.ndarray,
    portals: dict[tuple[int, int], list[tuple[int, int]]],
    portal_probabilities: dict[tuple[int, int], np.ndarray],
    conditioned_weight: float,
    likelihood_ratio_cap: float,
) -> float:
    """Mean log probability of the path's recognized ordered portal crossings."""
    path = np.asarray(nodes, dtype=int)
    recognized: list[tuple[int, int, int, int, int]] = []
    for u, v in zip(path[:-1], path[1:]):
        a, b = int(labels[int(u)]), int(labels[int(v)])
        options = portals.get((a, b), []) if a != b else []
        try:
            option_index = options.index((int(u), int(v)))
        except ValueError:
            continue
        recognized.append((a, b, int(u), int(v), int(option_index)))
    if not recognized:
        return 0.0
    source_coord, destination_coord = coords[int(path[0])], coords[int(path[-1])]
    log_probabilities = []
    total_steps = len(recognized)
    for step, (a, b, _u, _v, option_index) in enumerate(recognized):
        options = portals[(a, b)]
        fraction = float(step + 1) / float(total_steps + 1)
        target_coord = (1.0 - fraction) * source_coord + fraction * destination_coord
        ...
        public = np.exp((-distances / scale) - float((-distances / scale).max()))
        public /= float(public.sum())
        released = portal_probabilities.get((a, b))
        if released is None:
            raise RuntimeError("Matched scoring found a portal absent from the q5 layout")
        probabilities = portal_information_projection(
            public, released, conditioned_weight, likelihood_ratio_cap
        )
        log_probabilities.append(math.log(max(float(probabilities[option_index]), np.finfo(float).tiny)))
    return float(np.mean(log_probabilities))


def matched_candidate_order_sha256(arrays: dict[str, np.ndarray]) -> str:
    digest = hashlib.sha256()
    for name in ("slot_offsets", "node_offsets", "flat_nodes", "lengths", "log_errors", "base_scores"):
...
def pack_matched_candidate_bank(rows: list[list[tuple[np.ndarray, float, float, float]]]) -> dict[str, np.ndarray]:
    """Pack ordered per-slot proposals without pickle object overhead."""
    slot_offsets = [0]
    node_offsets = [0]
    flat_nodes: list[np.ndarray] = []
    lengths: list[float] = []
    log_errors: list[float] = []
    base_scores: list[float] = []
    for proposals in rows:
        for nodes, length, log_error, base_score in proposals:
            packed_nodes = np.asarray(nodes, dtype=np.int32)
            if packed_nodes.ndim != 1 or packed_nodes.size < 2:
                raise ValueError("Matched candidate paths must contain at least two nodes")
            flat_nodes.append(packed_nodes)
            node_offsets.append(node_offsets[-1] + int(packed_nodes.size))
            lengths.append(float(length))
            log_errors.append(float(log_error))
            base_scores.append(float(base_score))
        slot_offsets.append(len(lengths))
    arrays = {
        "slot_offsets": np.asarray(slot_offsets, dtype=np.int64),
        "node_offsets": np.asarray(node_offsets, dtype=np.int64),
        "flat_nodes": np.concatenate(flat_nodes).astype(np.int32, copy=False) if flat_nodes else np.empty(0, dtype=np.int32),
...
def write_matched_candidate_bank(path: Path, arrays: dict[str, np.ndarray], metadata: dict) -> str:
    payload = dict(arrays)
    payload["metadata_json"] = np.frombuffer(
        json.dumps(metadata, sort_keys=True, separators=(",", ":")).encode("utf-8"), dtype=np.uint8
    )
    payload["candidate_order_sha256"] = np.frombuffer(
        matched_candidate_order_sha256(arrays).encode("ascii"), dtype=np.uint8
    )
    temporary = path.with_suffix(path.suffix + ".tmp")
    with temporary.open("wb") as handle:
        ...
        os.fsync(handle.fileno())
    os.replace(temporary, path)
    return matched_candidate_order_sha256(arrays)


def load_matched_candidate_bank(path: Path, expected_metadata: dict) -> tuple[dict[str, np.ndarray], str]:
    with np.load(path, allow_pickle=False) as archive:
        arrays = {name: np.asarray(archive[name]) for name in (
            "slot_offsets", "node_offsets", "flat_nodes", "lengths", "log_errors", "base_scores"
        )}
        metadata = json.loads(bytes(np.asarray(archive["metadata_json"], dtype=np.uint8)).decode("utf-8"))
        recorded = bytes(np.asarray(archive["candidate_order_sha256"], dtype=np.uint8)).decode("ascii")
    if metadata != expected_metadata:
        raise RuntimeError("Matched candidate bank metadata/input binding mismatch")
    count = len(arrays["lengths"])
    if (
        arrays["slot_offsets"].ndim != 1
        or len(arrays["slot_offsets"]) != int(expected_metadata["limit"]) + 1
        or arrays["node_offsets"].ndim != 1
        or len(arrays["node_offsets"]) != count + 1
        or any(len(arrays[name]) != count for name in ("log_errors", "base_scores"))
        or int(arrays["slot_offsets"][0]) != 0
        or int(arrays["slot_offsets"][-1]) != count
        or int(arrays["node_offsets"][0]) != 0
        or int(arrays["node_offsets"][-1]) != len(arrays["flat_nodes"])
        or np.any(np.diff(arrays["slot_offsets"]) < 0)
        or np.any(np.diff(arrays["node_offsets"]) < 2)
        or not all(np.all(np.isfinite(arrays[name])) for name in ("lengths", "log_errors", "base_scores"))
    ):
        raise RuntimeError("Matched candidate bank structure is invalid")
    calculated = matched_candidate_order_sha256(arrays)
    if recorded != calculated:
        raise RuntimeError("Matched candidate order hash mismatch")
    return arrays, calculated


def categorical_index_from_uniform(probabilities: np.ndarray, uniform: float) -> int:
    cumulative = np.cumsum(np.asarray(probabilities, dtype=float))
    if cumulative.size == 0 or not np.isfinite(cumulative[-1]) or cumulative[-1] <= 0:
        raise ValueError("Categorical probabilities must have positive finite mass")
...
class RSPBridge:
    ...
    def choose_beta(self, source: int, destination: int, target_km: float) -> float:
        ...
        for beta in self.betas:
            ...
            errors.append(abs(expected - float(target_km)))
...
def main() -> None:
    ...
    matched_mode = production_mode and fixed_seed_mode
    if args.emit_matched_candidate_bank and args.matched_candidate_bank is not None:
        raise RuntimeError("Choose either candidate-bank emission or reuse, not both")
    if (args.emit_matched_candidate_bank or args.matched_candidate_bank is not None) and not matched_mode:
        raise RuntimeError("Candidate banks are restricted to fixed-seed matched research runs")
    if matched_mode:
        if args.route_alignment == "public-only" and not args.emit_matched_candidate_bank:
            raise RuntimeError("The matched public-only arm must emit the shared candidate bank")
        if args.route_alignment != "public-only" and args.matched_candidate_bank is None:
            raise RuntimeError("Matched shuffled/correct arms must reuse the public-only candidate bank")
    if args.emit_matched_candidate_bank and args.route_alignment != "public-only":
        raise RuntimeError("Only the public-only matched arm may generate the candidate bank")
    ...
    selection_uniforms = None
    selection_uniform_sha256 = None
    candidate_bank_arrays = None
    candidate_bank_rows = None
    candidate_order_sha256 = None
    candidate_bank_file_sha256 = None
    candidate_bank_metadata = None
    if matched_mode:
        selection_uniforms = np.random.default_rng(
            int(args.decoder_seed) ^ 0x4D54525F5135
        ).random(limit).astype("<f8", copy=False)
        selection_uniform_sha256 = hashlib.sha256(selection_uniforms.tobytes()).hexdigest()
        candidate_bank_metadata = {
            "schema_id": MATCHED_CANDIDATE_SCHEMA,
            "request_sha256": request_sha256,
            "dp_route_release_sha256": release_sha256,
            "public_osm_sha256": request_manifest["public_osm_sha256"],
            "limit": int(limit),
            "decoder_seed": int(args.decoder_seed),
            ...
            "candidate_generator": "public-only portal-fiber QRSP with fixed decoder stream",
        }
        if args.emit_matched_candidate_bank:
            candidate_bank_rows = []
        else:
            bank_path = args.matched_candidate_bank.resolve()
            if not bank_path.is_file():
                raise FileNotFoundError(f"Matched candidate bank is missing: {bank_path}")
            candidate_bank_arrays, candidate_order_sha256 = load_matched_candidate_bank(
                bank_path, candidate_bank_metadata
            )
            candidate_bank_file_sha256 = lineage.sha256_file(bank_path)
    ...
    for index in range(start_index, limit):
        ...
        if candidate_bank_arrays is not None:
            first = int(candidate_bank_arrays["slot_offsets"][index])
            last = int(candidate_bank_arrays["slot_offsets"][index + 1])
            for candidate_index in range(first, last):
                node_first = int(candidate_bank_arrays["node_offsets"][candidate_index])
                node_last = int(candidate_bank_arrays["node_offsets"][candidate_index + 1])
                nodes = candidate_bank_arrays["flat_nodes"][node_first:node_last].astype(int, copy=False)
                length = float(candidate_bank_arrays["lengths"][candidate_index])
                log_error = float(candidate_bank_arrays["log_errors"][candidate_index])
                base_score = float(candidate_bank_arrays["base_scores"][candidate_index])
                proposal_length_errors.append(log_error)
                proposals.append((nodes, {"candidate_source": "shared-public-only-candidate-bank"}, length, log_error))
                proposal_base_scores.append(base_score)
        else:
            ...
            for proposal_index in range(proposal_count):
                ...
                if nodes is not None and len(nodes) >= 2:
                    ...
                    proposal_base_scores.append(
                        candidate_portal_score(
                            np.asarray(nodes), context.node_fine, coords, portals,
                            portal_probabilities, conditioned_weight,
                            float(args.od_likelihood_ratio_cap),
                        ) if matched_mode else 0.0
                    )
        if candidate_path_augmenter is not None and candidate_bank_arrays is None:
            ...
            for extra_nodes, extra_state in extras:
                ...
                proposal_base_scores.append(
                    candidate_portal_score(
                        np.asarray(extra_nodes), context.node_fine, coords, portals,
                        portal_probabilities, conditioned_weight,
                        float(args.od_likelihood_ratio_cap),
                    ) if matched_mode else 0.0
                )
        if candidate_bank_rows is not None:
            candidate_bank_rows.append([
                (np.asarray(item[0], dtype=np.int32), float(item[2]), float(item[3]), float(base_score))
                for item, base_score in zip(proposals, proposal_base_scores)
            ])
        nodes = None
        ...
        if proposals:
            ...
            semantic_adjustments = np.zeros(len(proposals), dtype=float)
            if matched_mode:
                semantic_adjustments = np.asarray([
                    candidate_portal_score(
                        np.asarray(item[0]), context.node_fine, coords, portals,
                        portal_probabilities, conditioned_weight,
                        float(args.od_likelihood_ratio_cap),
                    )
                    - float(base_score)
                ...
                if not np.all(np.isfinite(semantic_adjustments)):
                    raise RuntimeError("Matched q5 semantic adjustments must be finite")
                logits = logits + semantic_adjustments
            probabilities = np.exp(logits - float(logits.max()))
            probabilities /= probabilities.sum()
            chosen_index = (
                categorical_index_from_uniform(probabilities, float(selection_uniforms[index]))
                if matched_mode
                else int(rng.choice(len(proposals), p=probabilities))
            )
            ...
            if matched_mode:
                current["matched_candidate_index"] = int(chosen_index)
                current["q5_semantic_logit_adjustment"] = float(semantic_adjustments[chosen_index])
    ...
    if candidate_bank_rows is not None:
        candidate_bank_arrays = pack_matched_candidate_bank(candidate_bank_rows)
        candidate_order_sha256 = write_matched_candidate_bank(
            staging / "candidate_bank.npz", candidate_bank_arrays, candidate_bank_metadata
        )
        candidate_bank_file_sha256 = lineage.sha256_file(staging / "candidate_bank.npz")
    beta_counts = {
    ...
    report = {
        ...
        "decoder_seed": int(args.decoder_seed) if args.decoder_seed is not None else None,
        "request_seed": int(args.request_seed) if args.request_seed is not None else None,
        "matched_candidate_contract": (
            {
                "schema_id": MATCHED_CANDIDATE_SCHEMA,
                "generation_arm": "public-only",
                "candidate_bank_sha256": candidate_bank_file_sha256,
                "candidate_order_sha256": candidate_order_sha256,
                "selection_uniform_sha256": selection_uniform_sha256,
                "selection_uniform_stream": "PCG64(decoder_seed XOR 0x4D54525F5135), one float64 per fixed slot",
                "score_formula": MATCHED_SCORE_FORMULA,
                "score_version": 1,
                "candidate_count": (
                    int(len(candidate_bank_arrays["lengths"]))
                    if candidate_bank_arrays is not None
                    else None
                ),
            }
        ...
        "release_object": {
            "coordinate_polyline": None,
            "road_witness_sidecar": "road_witnesses.pkl",
            "research_storage_note": "Matched attribution endpoints consume directed-edge witnesses; redundant coordinate and public-control copies are intentionally not serialized.",
            "witness_schema": {
        ...
        "implementation_source_sha256": {
            "quotient_rsp_bridge_matched.py": lineage.sha256_file(Path(__file__)),
            "portal_fiber_qrsp_production.py": lineage.sha256_file(
                Path(__file__).with_name("portal_fiber_qrsp_production.py")
            ),
            "portal_fiber_qrsp_production_gate.py": lineage.sha256_file(
                Path(__file__).with_name("portal_fiber_qrsp_production_gate.py")
            ),
        },
        ...
        "outputs": {
            ...
            **(
                {"candidate_bank.npz": candidate_bank_file_sha256}
                if candidate_bank_rows is not None
                else {}
            ),
        },
