...
- [x] Push to `origin/main` and verify a clean `core.autocrlf=true` checkout: `verify` (299 files), `smoke` (all synthetic artifacts), and `verify-matcher` (both executables) passed on commit `48ab840`. The final documentation-only follow-up commit also requires a final remote/manifest check.
